...
namespace MeteorSwathQueuedJobsSample
{
    ...
    class MeteorPath
    {
        ...
        private static string GetMeteorAPIPath() {
            ...
            for (UInt32 i = 0; i <= (runningAs32Bit ? 1 : 0); i++) {
                ...
                try {
                        ...
                        if (installKey != null) {
                            foreach (string subKeyName in installKey.GetSubKeyNames()) {
                                ...
                                if ( (fields.Length != 4) || !int.TryParse(fields[3], out thisRevision) ) {
                                    Logger.WriteLine(string.Format("MeteorPath: Invalid Meteor version key {0}", subKeyName));
                                } else {
                                    using (RegistryKey versionKey = installKey.OpenSubKey(subKeyName)) {
                                        object o = versionKey.GetValue("InstallPath");
                                        if ( o == null ) {
                                            Logger.WriteLine(string.Format("MeteorPath: Missing InstallPath for Meteor version {0}", subKeyName));
                                        } else if ( !(o is string) ) {
                                            Logger.WriteLine(string.Format("MeteorPath: Invalid InstallPath format for Meteor version {0}", subKeyName));
                                        } else {
                                            string dir = o as string;
                                            Logger.WriteLine(string.Format("MeteorPath: Found Meteor installation version = {0} path = '{1}'", subKeyName, dir));
                    ...
                    Logger.WriteLine("MeteorPath: Failed to access registry: " + e.Message);
                }
            }
        ...
        static List<string> MeteorAssemblies = new List<string>{
            "PrinterInterfaceCLS", "MeteorCLS", "UtilityClasses", "MeteorSwathInterfaceCLS", "MeteorSwathCLS"
        };
        ...
        static Assembly CurrentDomain_AssemblyResolve(object sender, ResolveEventArgs args) {
            ...
            if (MeteorAssemblies.Find(x => string.Compare(x, BaseName, true) == 0) != null) {
                ...
                if (path != null) {
                    path += "\\" + BaseName + ".dll";
                    if (File.Exists(path)) {
                        Logger.WriteLine("MeteorPath: Loading " + path);
        ...
        static private bool DoLocatePrinterInterface() {
            ...
            // API folder to the environment path of the current process.
                ...
                if (apipath != null) {
                    ...
                    Logger.WriteLine("MeteorPath: Adding '" + apipath + "' to local environment PATH");
                    ...
                    try {
                        ...
                        Logger.WriteLine("MeteorPath: Exception + " + e.Message);
                        return false;
                    }

                } else {
                    Logger.WriteLine("MeteorPath: Install folder not found");
                    return false;
                }
            }
